...
@section('title', 'Tambah Data Target')
@section('content')
	<section class="row">
		<div class="col-12">
			<div class="card">
				<div class="card-body py-4-5 px-4">
					<form action="{{ route('target.store') }}" method="POST">
						@csrf
						<div class="row mb-3">
							<div class="col-12 col-md-3"><label for="sosmed_id" class="mb-md-0 w-100 mb-2 text-start">Media Sosial</label></div>
							<div class="col-12 col-md-9">
								<select class="form-select @error('sosmed_id') border-danger @enderror" id="sosmed_id" name="sosmed_id" value="{{ old('sosmed_id') }}">
									@foreach ($sosmed as $item)
										<option value="{{ $item->id }}" {{ old('sosmed_id') == $item->id ? 'selected' : '' }}>{{ $item->sosmed }}</option>
									@endforeach
								</select>
							</div>
						</div>
